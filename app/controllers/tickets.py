    ...
    if "state" in data and data["state"] != old_state:
        ...
        if new_state and new_state.name == "done" and not was_done:
            ...
            if ticket.created_date and ticket.completed_date:
                lead_time = int(
                    (ticket.completed_date - ticket.created_date).total_seconds() / 60
                )
...
def archived_tickets() -> Any:
    """
    Render the archived tickets page.
    Returns:
        The rendered HTML template for archived tickets.
    """
    project_id = request.args.get("project_id", type=int)
    return render_template("tickets/archived.html", project_id=project_id)


@bp.route("/archived.json", methods=["GET"])
def get_archived_tickets_json() -> Any:
    """
    Get archived tickets as JSON, optionally filtered by project_id.
    Returns:
        A JSON response with archived tickets sorted by completed_date descending.
    """
    project_id = request.args.get("project_id", type=int)
    archived_state = TicketState.query.filter_by(name="archived").first()
    if not archived_state:
        return jsonify([])
    query = Ticket.query.filter_by(state=archived_state.id)
    if project_id:
        query = query.filter_by(project_id=project_id)
    tickets = query.order_by(Ticket.completed_date.desc().nullslast()).all()
    return jsonify([ticket.to_dict() for ticket in tickets])
